            ...
            try:
                ...
                if similarity >= SIMILARITY_THRESHOLD:
                    ...
                    if url and not url.startswith("http"):
                        # Example fallback: use topic ID and post ID to reconstruct URL
                        url = f"https://discourse.onlinedegree.iitm.ac.in/t/{chunk['topic_id']}/{chunk['post_id']}"
                    elif not url:
                        url = f"https://discourse.onlinedegree.iitm.ac.in/t/{chunk['topic_id']}/{chunk['post_id']}"
        ...
        if len(parts) > 1:
            ...
            for line in source_lines:
                ...
                if url_match:
                    ...
                    if text_match:
                        text_value = next((g for g in text_match.groups() if g), "").strip()
                        if text_value:
                            # Truncate to max 150 characters if too long
                            text = text_value if len(text_value) <= 150 else text_value[:147] + "..."
